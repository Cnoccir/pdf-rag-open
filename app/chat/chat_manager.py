"""
Fixed ChatManager for the PDF RAG system.
...
class ChatManager:
    ...
    def _get_query_graph(self):
        """Get or create query graph with recursion limit of 10"""
        if not self._query_graph:
            self._query_graph = create_query_graph({"recursion_limit": 10})
        return self._query_graph

    def _get_research_graph(self):
        """Get or create research graph with recursion limit of 10"""
        if not self._research_graph:
            self._research_graph = create_research_graph({"recursion_limit": 10})
    ...
    def query(self, query: str, pdf_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # Choose the appropriate graph with custom config including recursion_limit
        try:
            logger.debug("Creating LangGraph instance")
            graph_config = {"recursion_limit": 10}  # Explicitly set recursion limit
            graph = self._get_research_graph() if self.research_mode == ResearchMode.RESEARCH else self._get_query_graph()
            logger.debug(f"LangGraph instance created successfully with config: {graph_config}")
        ...
        try:
            ...
            with ThreadPoolExecutor() as executor:
                ...
                try:
                    ...
                    logger.error(f"Graph recursion limit exceeded: {str(recursion_error)}")
                    # Force-generate a fallback response for recursion errors
                    if self.conversation_state:
                        self.conversation_state.add_message(
                            MessageType.ASSISTANT,
                            "I'm having trouble processing your request due to its complexity. Could you please rephrase your question in a more specific way?"
                        )
                    ...
                    return {
                        "status": "error",
                        "query": query,
                        "error": "The system reached its processing limit. Please try a shorter or more specific question.",
                        "conversation_id": self.conversation_id,
                        "response": "I'm having trouble processing your request due to its complexity. Could you please rephrase your question in a more specific way?"
                    }
            ...
            if not result.generation_state or not result.generation_state.response:
                ...
                fallback_response = "I couldn't find specific information to answer your question. Could you please try rephrasing your question or ask about something else?"

                # Add fallback response to conversation if needed
                if self.conversation_state and not any(msg.type == MessageType.ASSISTANT and msg.content == fallback_response for msg in self.conversation_state.messages):
                    self.conversation_state.add_message(MessageType.ASSISTANT, fallback_response)
                    self.memory_manager.save_conversation(self.conversation_state)
            ...
            error_response = "I'm sorry, but I encountered a technical problem while processing your request. Please try again or contact support if the issue persists."

            # Add error response to conversation if needed
            if self.conversation_state:
                self.conversation_state.add_message(MessageType.ASSISTANT, error_response)
                self.memory_manager.save_conversation(self.conversation_state)
    ...
    def stream_query(self, query: str, pdf_ids: Optional[List[str]] = None) -> Generator[Dict[str, Any], None, None]:
        ...
        try:
            ...
            # Choose the appropriate graph with explicit recursion limit
            ...
            try:
                ...
                with ThreadPoolExecutor() as executor:
                    ...
                    try:
                        ...
                        if result and result.generation_state and result.generation_state.response:
                            ...
                            for i in range(0, len(fallback_response), self.stream_chunk_size):
                                chunk = fallback_response[i:i+self.stream_chunk_size]
                                yield {
                                    "type": "stream",
                                    "chunk": chunk,
                                    "index": i // self.stream_chunk_size,
                                    "is_complete": False,
                                    "percentage": 90 + (i / len(fallback_response)) * 10
                                }
                                time.sleep(0.05)

                    except TimeoutError:
                        logger.warning("Graph execution timed out")
                        fallback_response = "I'm taking too long to process your query. Could you please ask a simpler question?"
                        has_response = True
                        response_text = fallback_response
                        ...
                        logger.warning(f"Graph recursion limit reached: {str(recursion_error)}")
                ...
                fallback_response = "I encountered an error while processing your query. Please try again later."
                has_response = True
                response_text = fallback_response
            ...
            if has_response:
                ...
                # Fallback error
                fallback_response = "I couldn't generate a response to your question. Please try a different query."
            ...
            fallback_response = "I encountered an error while processing your request. Please try again later."
