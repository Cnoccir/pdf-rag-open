...
def load_model(Model: Model, extract_id_lambda=None, include_deleted=False):
    def decorator(view):
        ...
        def wrapped_view(**kwargs):
            ...
            try:
                # Modify find_by to include a check for soft delete (is_deleted)
                if include_deleted:
                    instance = Model.query.filter_by(id=model_id).first()
                else:
                    instance = Model.query.filter_by(id=model_id, is_deleted=False).first()
            except NoResultFound:
                raise ValueError(f"{Model.__name__} not found for id {model_id}.")

            if not instance:
                raise ValueError(f"{Model.__name__} not found or has been deleted.")
